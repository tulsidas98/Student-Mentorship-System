...
$query = "SELECT b.fname,fees_per_hr, No_of_hrs , (fees_per_hr * No_of_hrs) as Total_Session_Fees, slot_or_date as Session_Date,b.contact as Student_Contact FROM sessionmanagement as a,student_reg as b WHERE a.S_id=b.S_id and Sub_id=(select Sub_id from subject where Sub_name='$subject') and a.M_id = '$id'";
$result = mysqli_query($dbname, $query);
//echo "<br><br>";
echo "<h6 align='center' style= 'color: #FFFF00;'>$subject<br><br>";
echo"<table align='center' border ='1'>";

echo"<tr><td>Student Name</td><td>Fees Per hr</td><td>Session Duration (hrs)</td><td>Session Earning</td><td> Session_Date</td><td> Student_Contact</td></tr>";
while($row = mysqli_fetch_assoc($result))
{
	echo"<tr><td>{$row['fname']}</td><td>{$row['fees_per_hr']}</td><td>{$row['No_of_hrs']}</td><td>{$row['Total_Session_Fees']}</td><td>{$row['Session_Date']}</td><td>{$row['Student_Contact']}</td></tr>";
